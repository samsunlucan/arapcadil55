...
@Composable
fun SentenceListScreen(viewModel: SentenceViewModel, navController: NavController) {
    val items by viewModel.sentences.collectAsState()

    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        Button(onClick = { navController.navigate("sentence_form") }) {
            Text("Cümle Ekle")
        }

        LazyColumn(verticalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.padding(top = 8.dp)) {
            items(items) { item: Sentence ->
                Card(modifier = Modifier.fillMaxWidth()) {
                    androidx.compose.foundation.layout.Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            ...
                            Text(text = item.translation ?: "", style = MaterialTheme.typography.bodyMedium)
                        }
